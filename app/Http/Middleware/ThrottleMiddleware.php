...
class ThrottleMiddleware
{
    ...
    public function handle($request, Closure $next, $cost = 1)
    {
        ...
        if ($this->limiter->tooManyAttempts($key, $this->maxResources)) {
            $this->incrementTooManyAttemptsFor($key);

            return $this->buildTooManyAttempts($key, $this->maxResources);
        }
        ...
        return $this->addHeaders(
            $response, $this->maxResources,
            $this->calculateRemainingAttempts($key, $this->maxResources)
        );
    }
